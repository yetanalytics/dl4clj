(ns dl4clj.specs)
;;https://clojure.github.io/clojure/branch-master/clojure.spec-api.html#clojure.spec/keys*
;; convert these to specs
(defn typez
  [opts]
  (first (keys opts)))
...
(defmethod type-checking :writeable-converter [opts]
  (let [data (:writeable-converter opts)
        dt (type data)]
    (if (or (= org.datavec.api.io.converters.DoubleWritableConverter dt)
            (= org.datavec.api.io.converters.FloatWritableConverter dt)
            (= org.datavec.api.io.converters.LabelWriterConverter dt)
            (= org.datavec.api.io.converters.SelfWritableConverter dt))
      data
      (assert false "incorrect input type, should be a writeable converter"))))
